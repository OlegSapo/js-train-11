...
function setToArray(mySet) {
  ...
  let arr = [...mySet];
  // console.log(arr);
  let arrFiltered = arr.filter((elem) => typeof elem === "string");
...
function iterateSet(set) {
  ...
  let itrKey = set.keys();
  for (const key of itrKey) {
    console.log(key);
  }
  let itrValue = set.values();
  for (const value of itrValue) {
    console.log(value);
  }
  let itrEntrie = set.entries();
  for (const obj of itrEntrie) {
    console.log(obj);
  }
}
...
function sumNumbers(set) {
  ...
  let sum = 0;
  set.forEach((elem) => {
    typeof elem === "number" ? (sum += elem) : (sum += 0);
  });
  return sum;
}
